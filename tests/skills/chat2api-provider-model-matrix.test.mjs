...
test('matrix runner dry-run does not leak configured secrets', () => {
  const result = spawnSync('node', [script, '--fixture', 'fixture.json', '--dry-run'], {
    env: {
      ...process.env,
      CHAT2API_API_KEY: 'placeholder-api-key',
      CHAT2API_MGMT_SECRET: 'placeholder-management-secret',
    },
    encoding: 'utf8',
  })

  assert.equal(result.status, 0, result.stderr)
  assert.doesNotMatch(result.stdout, /placeholder-api-key/)
  assert.doesNotMatch(result.stdout, /placeholder-management-secret/)
})
...
`)
  ...
  assert.equal(fetchCall.headers.Authorization, 'Bearer placeholder-api-key')
  ...
  assert.doesNotMatch(JSON.stringify(report), /placeholder-api-key/)
  assert.doesNotMatch(JSON.stringify(report), /placeholder-management-secret/)
})
...
`)
  ...
  assert.doesNotMatch(result.stderr, /placeholder-api-key/)
  assert.doesNotMatch(result.stderr, /placeholder-management-secret/)
})
