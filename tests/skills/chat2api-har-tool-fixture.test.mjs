...
test('uses env fallback, exact chat path selection, and body secret redaction', () => {
  ...
  assert.doesNotMatch(JSON.stringify(fixture), /placeholder-api-key|placeholder-token|placeholder-password|secret-cookie/)
})
...
test('redacts secret values from retained allowed headers', () => {
    ...
    log: {
      entries: [{
        request: {
          ...
          url: 'http://127.0.0.1:8080/v1/chat/completions',
          headers: [
            { name: 'x-title', value: 'Bearer placeholder-header-token' },
            { name: 'User-Agent', value: 'CherryStudio/1.9.6' },
          ],
          postData: {
  ...
  assert.doesNotMatch(JSON.stringify(fixture), /placeholder-header-token/)
})
